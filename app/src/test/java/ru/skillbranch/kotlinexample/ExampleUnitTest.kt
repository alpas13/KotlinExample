...
class ExampleUnitTest {
    ...
    fun import_csv_file() {
        val holder = UserHolder
        val numberOfUsers = holder.importUsersFromFile()

        val expectedNumbersOfUsers = 12
        val expectedInfo = """
            firstName: Peter
            lastName: Stanbridge
            login: peter-stanbridge@unknown.com
            fullName: Peter Stanbridge
            initials: P S
            email: peter-stanbridge@unknown.com
            phone: null
            meta: {auth=password}
        """.trimIndent()

        val successResult =  holder.loginUser("peter-stanbridge@unknown.com", Date().toString())

        Assert.assertEquals(expectedNumbersOfUsers, numberOfUsers)
        Assert.assertEquals(expectedInfo, successResult)
    }
    ...
    fun drop_last_until() {
        ...
        val successResult =  testList.dropLastUntil("Tamzyn French")
